...
| `play_amp_animation.py` crashes: `RuntimeError: operator torchvision::nms does not exist` | Container built from stale image (before Fix 1/Fix 2); Fix 2 previously used wrong ext key `omni.isaac.lab_tasks` instead of `isaaclab_tasks` | Rebuild the image — Dockerfile now patches `stack_ik_rel_blueprint_env_cfg.py` (Fix 1) and disables the extension with key `isaaclab_tasks` (Fix 2) |
